...
WebDAV(Koofr)는 계속 켜둔 채로 유지한다. Syncthing과 다른 층에서 동작해서
(Syncthing은 두 컴퓨터 직접 동기화, WebDAV는 클라우드 백업) 서로 충돌하지 않고,
오히려 클라우드에 별도 백업 사본이 하나 더 생기는 셈이라 이득. 아주 드물게
두 프로그램이 같은 파일을 거의 동시에 건드리면 Syncthing이 `*.sync-conflict-*.pdf`
파일을 만들어 두는 정도이며 데이터 손실은 없다.
...
5. WebDAV(Koofr) File Syncing은 그대로 유지 — 끄지 않는다.
